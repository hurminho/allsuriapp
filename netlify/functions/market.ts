...
async function enqueueAfterBid(opts: {
  listingId: string
  businessId: string
  bidAmount: unknown
}) {
  ...
  try {
    ...
    clearTimeout(timer)
    console.log(`[market] after-bid background invoke status=${res.status}`)
    if (res.status === 202 || res.ok) return
    console.warn('[market] after-bid background not accepted, running detached')
  } catch (e: any) {
    clearTimeout(timer)
    console.warn('[market] after-bid background invoke error:', e?.message)
  }
  void afterBidInserted(opts).catch((e: any) =>
    console.warn('[market] bid side effects failed:', e?.message),
  )
}
